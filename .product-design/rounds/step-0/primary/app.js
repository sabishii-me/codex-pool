...
function renderSpline(target, primary, secondary, options = {}) {
  ...
  target.innerHTML = `<svg class="spline-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="${gradient}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#f87171" stop-opacity=".28"/><stop offset="1" stop-color="#f87171" stop-opacity="0"/></linearGradient></defs><path d="${areaPath}" fill="url(#${gradient})"/><path class="spline-secondary" d="${secondaryPath}"/><path class="spline-primary" d="${primaryPath}"/><line class="spline-marker" x1="${activeX}" y1="${top}" x2="${activeX}" y2="${height-bottom}"/><circle class="spline-node-primary" cx="${activeX}" cy="${activeY}" r="4"/><circle class="spline-node-secondary" cx="${activeX}" cy="${secondaryY}" r="3.5"/><g transform="translate(${tooltipX} ${tooltipY})"><rect class="spline-tooltip" width="108" height="42" rx="12"/><circle cx="12" cy="14" r="3" fill="#f87171"/><text class="spline-tooltip-text" x="20" y="17">${primary[active]} ${options.primaryUnit || "req"}</text><circle cx="12" cy="29" r="3" fill="#fde047"/><text class="spline-tooltip-sub" x="20" y="32">${secondary[active]} benchmark</text></g>${yLabels.map((value,index)=>`<text class="spline-axis" x="2" y="${top+index*(height-top-bottom)/2+3}">${Math.round(value)}</text>`).join("")}${xLabels.map((label,index)=>`<text class="spline-axis" text-anchor="middle" x="${left+index*(width-left-right)/(xLabels.length-1)}" y="${height-8}">${label}</text>`).join("")}</svg>`;
}
