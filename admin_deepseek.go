...
func (h *proxyHandler) serveDeepSeekAdmin(w http.ResponseWriter, r *http.Request) {
	path := strings.TrimPrefix(r.URL.Path, "/admin/deepseek")
	if path == "" {
		path = "/"
	}
	...
	case path == "/" && r.Method == http.MethodGet:
		h.handleAPIKeyList(w, AccountTypeDeepSeek)
	...
	case strings.HasSuffix(path, "/remove") && r.Method == http.MethodPost:
		...
		h.handleAPIKeyRemove(w, AccountTypeDeepSeek, id)
	default:
		http.NotFound(w, r)
	}
}

func (h *proxyHandler) handleDeepSeekAdd(w http.ResponseWriter, r *http.Request) {
	var req struct {
	...
	validationURL := h.cfg.deepseekBase.String() + "/v1/messages"
	body := map[string]any{
	...
	h.saveAPIKeyAccountFile(w, AccountTypeDeepSeek, "deepseek", apiKey)
}
