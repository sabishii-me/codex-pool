...
func (h *proxyHandler) serveOpenRouterAdmin(w http.ResponseWriter, r *http.Request) {
	path := strings.TrimPrefix(r.URL.Path, "/admin/openrouter")
	if path == "" {
		path = "/"
	}
	...
	case path == "/" && r.Method == http.MethodGet:
		h.handleAPIKeyList(w, AccountTypeOpenRouter)
	...
	case strings.HasSuffix(path, "/remove") && r.Method == http.MethodPost:
		...
		h.handleAPIKeyRemove(w, AccountTypeOpenRouter, id)
	default:
		http.NotFound(w, r)
	}
}

func (h *proxyHandler) handleOpenRouterAdd(w http.ResponseWriter, r *http.Request) {
	var req struct {
	...
	validationURL := h.cfg.openrouterBase.String() + "/v1/messages"
	body := map[string]any{
	...
	h.saveAPIKeyAccountFile(w, AccountTypeOpenRouter, "openrouter", apiKey)
}
