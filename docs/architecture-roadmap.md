...
Status: in progress. The provider-neutral `ConnectionIdentity` seam is implemented with durable `display_name`, optional `external_subject`/attributes, migration-safe legacy fallback, normalized stats/operator DTOs, an elevated-operator rename endpoint, and a compatibility-free `/api/v2/provider-connections` contract. `GatewayUser`, `ProviderID`, `ProviderConnection`, `ProviderPool`, and `ModelRoute` are now the canonical declared types used by the provider contract, catalog, and TypeScript view-model consumers. Live usage recording now writes `ConnectionID`/`ProviderID`; legacy `AccountID`/`AccountType` fields are normalized only for old Bolt events, projections, and source compatibility. Existing `PoolUser`, `AccountType`, `Account`, `poolState`, compatibility JSON fields, and `/admin/accounts` URLs remain deprecated aliases/interfaces until compiler-guided call-site and API-version migrations are complete.
