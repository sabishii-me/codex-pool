...
Status: in progress. Anthropic Messages and OpenAI Chat usage engines are extracted protocol capabilities. Claude, DeepSeek, Z.ai, MiniMax, Qwen, OpenRouter, Kimi, Kimi Platform, and Xiaomi delegate Anthropic streaming/non-streaming normalization to one implementation. NVIDIA and both Kimi adapters delegate OpenAI Chat normalization with an explicit cache-billing policy, preserving Kimi's legacy accounting rather than silently changing totals. Mixed-protocol adapters retain only genuine fallback formats. Phase 0 provider contracts remain the non-regression boundary.
