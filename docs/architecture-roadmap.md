...
Status: in progress. Anthropic Messages, OpenAI Chat, and OpenAI Responses usage engines are extracted protocol capabilities. Claude, DeepSeek, Z.ai, MiniMax, Qwen, OpenRouter, Kimi, Kimi Platform, and Xiaomi delegate Anthropic streaming/non-streaming normalization to one implementation. NVIDIA and both Kimi adapters delegate OpenAI Chat normalization with an explicit cache-billing policy, preserving Kimi's legacy accounting rather than silently changing totals. Codex, Grok, and sampled response accounting delegate Responses normalization with explicit options for Chat aliases, cache writes, upstream billable totals, and billable-only events; Codex retains its custom `token_count` fallback. Mixed-protocol adapters retain only genuine fallback formats. Phase 0 provider contracts remain the non-regression boundary.
