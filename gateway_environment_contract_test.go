...
func TestRuntimeHasNoBehaviorBypassSwitches(t *testing.T) {
	forbidden := regexp.MustCompile(`(?i)(PROXY_DISABLE_REFRESH|disable_refresh|LOCAL_DEV_SESSION|DEV_LOCAL_SESSION|STAGING_LOCAL_SESSION|PROXY_LOG_BODIES|PROXY_BODY_LOG_LIMIT|PROXY_CLAUDE_TRACE|PROXY_IMAGE_TRACE|PROXY_DEBUG|DEV_PROXY_DEBUG|STAGING_PROXY_DEBUG|DEV_PROXY_DISABLE_REFRESH|STAGING_PROXY_DISABLE_REFRESH|DEV_PROXY_MAX_ATTEMPTS|STAGING_PROXY_MAX_ATTEMPTS|DEV_PROXY_USAGE_REFRESH_SECONDS|STAGING_PROXY_USAGE_REFRESH_SECONDS|UPSTREAM_KIMI_PLATFORM_BASE|DEV_UPSTREAM_KIMI_PLATFORM_BASE|STAGING_UPSTREAM_KIMI_PLATFORM_BASE|WEBSOCKET_COMPRESSION|CODEX_TLS_FINGERPRINT|CODEX_FINGERPRINT_AUTO_UPDATE|CODEX_FINGERPRINT_UPDATE_SECONDS|TIER_THRESHOLD|tier_threshold)`)
