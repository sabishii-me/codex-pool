...
func TestStagingAndDevelopmentComposeStayIsolated(t *testing.T) {
	...
	for _, required := range []string{"${STAGING_IMAGE:?set STAGING_IMAGE to the immutable image promoted from Test}", "staging-data-permissions:", "chown -R codex:codex /app/data /app/pool /app/provider-specs", "condition: service_completed_successfully", "127.0.0.1:18990:8989", "./staging/pool:/app/pool", "./staging/data:/app/data", "LOCAL_DEV_SESSION: ${STAGING_LOCAL_SESSION:-false}", "${STAGING_"} {
