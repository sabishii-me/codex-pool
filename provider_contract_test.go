...
func providerContractMatrix() []providerContract {
	...
	return []providerContract{
		...
		{NewGrokProvider(openAI), "openai-responses-custom", "model", verified("Responses usage parser coverage"), verified("Responses JSON proxy integrity contract"), verified("cached token details parsed"), notApplicable("protocol usage exposes cache reads but no cache-creation dimension"), verified("reasoning token details parsed"), verified("translated request sanitization covered"), verified("oversized requests explicitly rejected before unsafe partial sanitization"), verified("canonical event asserted once with root request ID and attribution")},
...
func TestProviderContractMatrixCoversRegistry(t *testing.T) {
	...
	wantTypes := []AccountType{
		AccountTypeCodex, AccountTypeGemini, AccountTypeAntigravity,
