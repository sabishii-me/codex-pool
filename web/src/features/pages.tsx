...
export function Page({ route, stats, signal, models, connections, users, health, session, onNavigate }: { route: string; stats: PoolStats | null; signal: SignalAnalytics | null; models: ModelDescriptor[]; connections: OperatorProviderConnectionV2[]; users: PoolUserStats[]; health: GatewayHealth | null; session: FriendSession; onNavigate: (path: AppRoute) => void }) {
  ...
  if (route === "/usage" || route === "/operator/usage") return <UsagePage stats={stats} signal={signal} operator={route.startsWith("/operator")} originId={session.origin_id} />;
  ...
  if (route === "/operator/system") return <OperatorPage title="System" description="Runtime, persistence, configuration, and recovery readiness." rows={health ? [{ title: "Gateway runtime", meta: `Uptime ${health.uptime}`, state: health.status === "ok" ? "healthy" : "degraded", detail: `Health endpoint: ${health.status}` }] : []} empty="System health is unavailable." />;
  if (route === "/operator/monitor") return <MonitorPage stats={stats} signal={signal} />;
  return <DashboardPage stats={stats} signal={signal} models={models} operator={route === "/operator"} originId={session.origin_id} onNavigate={onNavigate} />;
}
